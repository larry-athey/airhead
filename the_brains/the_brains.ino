...
float CorrectionFactor = 3.0;    // How much to reduce TempC readings to reflect internal temperatue
    ...
    if (ActiveRun) {
      ...
      if (CurrentMode > 1) { // Mode 1 is constant power, no temperature management
        if (! UpToTemp) {
          ...
          if (CurrentTime - FallBackTime >= 60000) { // Wait 1 minute for the temperature to level out after the 33% fall-back
