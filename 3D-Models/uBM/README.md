...
Unlike the full Boilermaker, these are sold on my website as 120-volt-only and bench/counter-top-only. The size of the case only allows for 14 gauge grounded power cables and they only ship with an SSR-40DA solid state relay mounted to the same heat sink used in the Airhead. These are priced the same as the full Boilermaker with 120 volt pigtails.
